...
def calcQSAR(ch, atom, MolWt, model_data, doLrp=True):
   ...
   if doLrp == False:
    dummy_scores = np.zeros(1, dtype=np.float32)
    dummy_bias = 0.0
    dummy_ls = [0.0]

    return l_out[0], dummy_scores, dummy_bias, dummy_ls
   ...
   b=LRPCheck("HighWay Output:", R_highway, l_out);

   R_identity, R_transformed_gated = calcLRPAddition(identity_gated, transformed_gated, l_highway, R_highway);
   #b=LRPCheck("Identity Gated:", [R_identity, R_transformed_gated], R_highway);
   ...
   b=LRPCheck("Input HighWay:", R_input_highway, R_highway);

   R_cnn = calcLRPDenseInner(l_cnn, [d[145], d[146]], R_input_highway);
   #b=LRPCheck("CNN concat:", R_cnn, l_out);
...
def mycalc(model_data, mol):
    ...
    if num_atoms > 50:
       ...
       do_lrp = True
    for atom in range(num_atoms):
        val, scores, bias, ls = calcQSAR(m,atom,MolWt,model_data,doLrp=do_lrp)
        ml.append(ls[0])
        vals.append(val);
    ...
    if res>=0.5:
       ...
       act='Inactive'
    svg=figure_process(mol,m,labels,impacts)
    mlh=[i for i in ls if i>50]
    return act,svg,len(mlh),len(ml)

#print("{} = {:.7f} +/- {:7f} {}".format(info[0], res, 1.96*std / math.sqrt(len(vals)), info[3]));

def figure_process(mol,m,labels,impacts):
